        ...
        <div className="w-9/10 lg:w-1/2 md:w-1/2  flex flex-col place-content-between">
          ...
          <div className=" w-full h-2/3 mx-10 my-5">
            <div className="h-1/5 w-full  flex flex-col gap-1">
              <h3 className="text-lg font-mono">Sign Up</h3>
            ...
            <form className="flex flex-col gap-3" onSubmit={(e)=>{handleSubmit(e)}}>
              <div className="flex flex-col gap-1 items-start">
                <label className="flex items-start"><span className="text-red-500 text-[7px] ">&#10040;</span>Username</label>
                <input value={username} onChange={(e)=>{
                    setUsername(e.target.value)
                }} className="w-9/10 px-2 py-1 border border-black rounded-sm outline-0 hover:border-blue-600" type="text" placeholder="Enter Your Username" />
              </div>
              <div className="flex flex-col gap-1 items-start">
                <label className="flex items-start"><span className="text-red-500 text-[7px] ">&#10040;</span>Email</label>
                <input value={email} onChange={(e)=>{
                    setEmail(e.target.value)
                }} className="w-9/10 px-2 py-1 border border-black rounded-sm outline-0 hover:border-blue-600" type="email" placeholder="Enter Your Email" />
              </div>
              <div className="flex flex-col gap-1">
                ...
                <button className="w-9/10 bg-blue-600 text-white p-2 rounded-2xl mt-2" disabled={loading}>{loading? "Signing up.." : "Sign up"}</button>
                <p className="text-xs w-9/10 text-gray-600 text-center">Already have an Account ? <Link to='/login' className="text-xs text-blue-600">Log In</Link></p>
            </form>
          </div>
